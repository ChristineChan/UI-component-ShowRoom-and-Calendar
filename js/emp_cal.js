...
$(function(){
	...
	$(".type-box").live("click",function(){
		...
		if(index>=nLength){
			...
			$(".box").each(function(){
				if(a<42){
//                                        var jsonDate = Content.Json[index].data[a].time;
//                                        var boxDate = new Date(jsonDate);
//                                        var nowDate = new Date();
//                                        if(boxDate>nowDate){
                                            $(this).find("ul").append(template(Content.Json[index].data[a]));
//                                            $(this).addClass('day-future');
//                                        }else{
//                                            $(this).addClass('day-pass');
//                                        }
                                        a++;
				}
			});
                        $('.day-pass').css('height',$('.day-future').css('height'));
			roomShow.push(index);
